...
int main()
{
    ...
    int shared_array[ARRAY_SIZE]; // this is main memory, can assign data to it @me
...
#pragma omp parallel num_threads(NUM_PROCS)
    {
        ...
        {
            // Check cache state
            if (cache_state[thread_id][i] == INVALID || cache_state[thread_id][i] == SHARED) // can discard SHAREED condition but in some cases its nessasry to read from memmory @me
            {
            ...
            {
                // Data already in cache, use cache value
                // No need to update cache state
            }

            // Perform some computation on the data (e.g., increment by thread ID)
            cache[thread_id][i] += thread_id;
            cache_state[thread_id][i] = MODIFIED; // @me

            // Update shared array if cache state is MODIFIED
            if (cache_state[thread_id][i] == MODIFIED)
            {
                shared_array[i] = cache[thread_id][i]; // updating the main memory @me
            }

            // Update cache state to SHARED
            cache_state[thread_id][i] = MODIFIED;
        }
    }
